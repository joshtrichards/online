...
bool ClientSession::handleKitToClientMessage(const std::shared_ptr<Message>& payload)
{
    ...
    if (!isDocPasswordProtected())
    {
        ...
        else if (tokens.equals(0, "filleddocumentstructure:"))
        {
            LOG_TRC("Sending filled document structure response.");
            if (!_saveAsSocket)
                LOG_ERR("Error in filleddocumentstructure: not in isConvertTo mode");
            else
            {
                const std::string stringJSON = payload->jsonString();
            ...
            // Now terminate.
            docBroker->closeDocument("filleddocumentstructure");
            return true;
        }
